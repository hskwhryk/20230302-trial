...
observe({
  ...
  #tmpynames = data_analyzed$seqdata.cpm%>%select(-unique_id,-sampleid,-group,-one_of("rep"))%>%colnames()
  tmpynames <- c("cpm", "log2.cpm")
...
output$dotplot <- renderPlotly({
  ...
  # data_analyzed = analyzeDataReactive()
  seqdata.cpm = data_analyzed$seqdata.cpm
  seqdata.log2cpm = data_analyzed$seqdata.log2cpm
  seqinfo = data_analyzed$seqinfo
  geneids = data_analyzed$geneids
  if (names(dev.cur()) != "null device") dev.off()
  pdf(NULL)
  p=dotplot_fun(data.cpm = seqdata.cpm,
                data.log2cpm = seqdata.log2cpm, 
                seqinfo = seqinfo,
                geneids = geneids,
...
DataDotplotReactive <- reactive({
  ...
  subdat = dotplot_dat(seqdata.cpm = data_analyzed$seqdata.cpm,
                       geneids = data_analyzed$geneids,
